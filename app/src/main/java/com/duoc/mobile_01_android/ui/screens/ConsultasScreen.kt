...
fun ConsultasScreen(
    ...
    fun calcularTotal(): Double {
        return viewModel.calcularTotalConsulta(selectedMedicamentos.toList())
    }
    ...
    Scaffold(
        ...
        AnimatedVisibility(
            ...
            Column(
                ...
                Text(
                    text = "Lista de Consultas (${consultas.size})",
            ...
            AlertDialog(
                ...
                text = {
                    Column(
                        ...
                        // Selector de Mascota (filtrado por cliente)
                        val mascotasFiltradas = selectedCliente?.let { cliente ->
                            mascotas.filter { it.clienteId == cliente.id }
                        } ?: emptyList()
                        ...
                        Text(
                            text = "Costo base consulta: $${String.format("%,.0f", MainViewModel.COSTO_BASE_CONSULTA)}",
                            style = MaterialTheme.typography.bodyMedium
                        )
                        Text(
                        ...
                        Text(
                            text = "TOTAL: $${String.format("%,.0f", calcularTotal())}",
                            ...
                            selectedCliente?.let { cliente ->
                                selectedMascota?.let { mascota ->
                                    viewModel.crearConsulta(
                                        cliente = cliente,
                                        mascota = mascota,
                                        medicamentosSeleccionados = selectedMedicamentos.toList(),
